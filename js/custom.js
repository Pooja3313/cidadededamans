  ...
  (function () {
    ...
    wind.on("scroll", function () {
      ...
      if (bodyScroll > 100) {
        ...
        logo.attr("src", "img/banner/Cidade_De_Daman_Logo.jpg");
      } else {
        navbar.removeClass("nav-scroll");
        logo.attr("src", "img/banner/cidadededaman-logo-white.jpg");
      }
    });
...
//  Destination and Dream Wedding images index.js keh liye slide keh liye texting

const textElement = document.getElementById("changing-text");
const words = ["Dream Wedding", "Destination Wedding"];
let i = 0;
...
window.addEventListener("DOMContentLoaded", function () {
  openPopup(); // Show on page load

  // 1st display: close after 3 seconds
  setTimeout(() => {
    closePopup();
  }, 5000);

  // Final close: after 8 seconds total
  setTimeout(() => {
    closePopup();
  }, 8000);
});
...
document.addEventListener("DOMContentLoaded", function () {
  const reviewForm = document.getElementById("review-forms");
  ...
  if (reviewForm) {
    reviewForm.addEventListener("submit", async function (e) {
      ...
      const title = reviewForm.elements["title"].value.trim();
      ...
      const isGenuine = document.getElementById("genuine").checked;
      ...
      // Validation
      if (!rating || !title || !review || !name || !email || !isGenuine) {
        Toastify({
          text: "All fields are required and checkbox must be checked!",
          duration: 1000,
          gravity: "top",
          position: "right",
          style: {
      ...
      try {
        const response = await fetch("http://localhost:3000/api/review", {
          method: "POST",
          headers: { "Content-Type": "application/json" },
          body: JSON.stringify({ rating, title, review, name, email }),
        });
